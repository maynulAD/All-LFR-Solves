...
void readLine()
{
  ...
  //     arf[scan_order_f[i]] = constrain(map(sensorRead(f5[scan_order_f[i]]),sminf[scan_order_f[i]],smaxf[scan_order_f[i]],-30,100), 0,100);
  //     // if(arf[scan_order_f[i]]<0) arf[scan_order_f[i]] = 0;
  //     if(i==2){drf[scan_order_f[i]] = arf[scan_order_f[i]] < 5?0:1;}
  //     else{drf[scan_order_f[i]] = arf[scan_order_f[i]] < 30?0:1;}
  // }
  for(int i=0; i<NUM_SENSORS_B; i++){
      arb[scan_order_b[i]]=constrain(map(sensorRead(b9[scan_order_b[i]]),sminb[scan_order_b[i]],smaxb[scan_order_b[i]],-5,15), 0,10);
      if(line=='W'){
         arb[scan_order_b[i]]=10-arb[scan_order_b[i]];
         drb[scan_order_b[i]] = arb[scan_order_b[i]] < (10-3)?0:1;
      }else{
          drb[scan_order_b[i]] = arb[scan_order_b[i]] < 3?0:1;
          }
  }
}
...
{
  String pattern="";
  // patt5=pattern+drf[0]+drf[1]+drf[2]+drf[3]+drf[4];
...
long getPos(int n)
{
  ...
  weight=max_spd;
  if(DEBUGGING) oled_printBar(arb,4-(n/2),4+(n/2),10);
