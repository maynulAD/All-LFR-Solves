...
void turnDeg(char dir, int deg, bool adjust)
{
  ...
  if(dir=='R')rightHandAdjust=20;

  if (dir == 'L') setMotors(-tSpd, tSpd);
  else if(dir == 'R') setMotors(tSpd, -tSpd);
  if(deg==45){
    delay(80+rightHandAdjust);
  }else if(deg==90){
    delay(150+rightHandAdjust);
  }else if(deg==135){
    delay(215+rightHandAdjust);
  }else if(deg==180){
    delay(310);
  }
  if(adjust==true){while(getPattern(9) == "000000000");}
  setMotors(0,0);
  dir='S';
}
...
{
  int bSpd;
  bSpd=map( (runtime), 180,800, 10,-40 );
  bSpd=constrain(bSpd*0.75,-40,10);
  setMotors(bSpd,bSpd);
