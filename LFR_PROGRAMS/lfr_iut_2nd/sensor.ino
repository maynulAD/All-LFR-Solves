...
void readLine()
{
  ...
  for(int i=0; i<NUM_SENSORS_F; i++){ 
    tmp = map(sensorRead(f5[scan_order_f[i]]) , sminf[scan_order_f[i]],smaxf[scan_order_f[i]], -3 *_F, 15 *_F);
  ...
  for(int i=0; i<NUM_SENSORS_B; i++){
    tmp = map(sensorRead(b9[scan_order_b[i]]),sminb[scan_order_b[i]],smaxb[scan_order_b[i]], -2 *_F, 17 *_F);
