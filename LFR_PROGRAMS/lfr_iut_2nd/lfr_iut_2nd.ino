...
void loop() {
  int tCount = 0, yCount = 0, count = 0, bSpd;
  bool obj = false, ramp=false, flagY=true;
  bool wallPassed = false, wlinePassed = false, skipOnce=true;
  ...
  while(1){
    ...
    if( flagY &&  (patt55=="01110" || patt55=="01010") ){
      yCount++;
      if(yCount==2)flagY=false;
      setMotors(base_spd,base_spd);delay(80);
      turn(hand);
      continue;
    }
    ...
    if(ramp){
      max_spd=rampUpSpd;
      for(int i=0;i<1200;i++){readLine();pos=getPos(7);pid(1);}
    ...
    if( (!drb[1]) && (drb[3]||drb[4]||drb[4]) && (drb[7]) ){ //  right
      ...
      if(getPattern(5)=="00000"){ // only left
        ...
        if(hand=='L'){
          /*  skip first time  */
          // if(line=='B' && skipOnce){
          //   for(int i=0;i<160;i++){readLine();pos=getPos(7);pid(1);}
          //   skipOnce=false;
          //   continue;
          // }
          /*  skip first time  */
