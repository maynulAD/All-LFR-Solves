...
#define ENDPATT ( (!drf[0]&&!drf[1] && !drb[0]&&!drb[1]&&!drb[2]) && (drf[2]&&drf[3]&&drf[4] && drb[4]&&drb[5]&&drb[6]&&drb[7]&& drb[8]) )
...
void loop() {
  int tCount = 0, yCount = 0, count = 0, uCount=0, gCount=0;
...
//      runtime = millis()-lastMillis; delay(25); brake(runtime);
...
//          turnDeg('R',45);turn('R',7);   // use 7 sensor
...
//        continue;
        ...
        halt();oled.clear();oled_print(0,0,"XX                 XX");
        turnPID(dir);  // use 7 sensor
        dir='S';
        lastMillis=millis();
        continue;
      }
