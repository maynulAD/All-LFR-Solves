...
void readLine()
{
  ...
  for(int i=0; i<NUM_SENSORS_F; i++){
    if(line=='W'){
      ...
      drf[scan_order_f[i]] = arf[scan_order_f[i]] < (10-2)*_F ? 0:1;
    }else{
      tmp = map(sensorRead(f5[scan_order_f[i]]) , sminf[scan_order_f[i]],smaxf[scan_order_f[i]], -5 *_F, 17 *_F);
      arf[scan_order_f[i]] = constrain(tmp, 0,10*_F);
  ...
  for(int i=0; i<NUM_SENSORS_B; i++){
    if(line=='W'){
      tmp = map(sensorRead(b9[scan_order_b[i]]),sminb[scan_order_b[i]],smaxb[scan_order_b[i]], -3 *_F, 17 *_F);
      arb[scan_order_b[i]]=constrain(tmp, 0,10*_F);
      arb[scan_order_b[i]]=10*_F - arb[scan_order_b[i]];
      drb[scan_order_b[i]] = arb[scan_order_b[i]] < (10-3)*_F ? 0:1;
    }else{
      tmp = map(sensorRead(b9[scan_order_b[i]]),sminb[scan_order_b[i]],smaxb[scan_order_b[i]], -3 *_F, 17 *_F);
      arb[scan_order_b[i]]=constrain(tmp, 0,10*_F);
...
long getPos(int n)
{
  ...
  if(n==5){
    ...
    w_sum=arb[0]*0+arb[1]*weight*0.25+arb[2]*weight*0.50+arb[3]*weight*0.75+arb[4]*weight+arb[5]*weight*1.25+arb[6]*weight*1.50+arb[7]*weight*1.75+arb[8]*weight*2;
  }
  _pos=w_sum/sum;
